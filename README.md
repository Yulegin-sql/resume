# resume

SQL Developer

\# Привет! 👋 Я Глеб



\### 🗄️ Начинающий аналитик данных / SQL-разработчик



Изучаю SQL и аналитику данных. Собираю портфолио реальных проектов: 

от простых SELECT-запросов до оконных функций и оптимизации.



\---



\## 🛠️ Мой стек



![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)

![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)

![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)



\---



\## 📚 Что я уже умею



\- ✅ SELECT, WHERE, JOIN (INNER, LEFT, RIGHT, FULL)

\- ✅ GROUP BY, HAVING, агрегатные функции

\- ✅ Подзапросы и CTE (`WITH`)

\- ✅ Оконные функции (`ROW\_NUMBER`, `RANK`, `LAG`, `LEAD`)

\- ✅ Создание и нормализация таблиц (DDL)

\- ✅ Индексы и базовая оптимизация запросов



\---



## 🚀 Мои проекты

| Проект | Описание | Стек |
| :--- | :--- | :--- |
| [📊 SQL Practice](https://leetcode.com/u/zengas9/) | 100+ решённых задач с LeetCode | SQL |
| [🛒 Анализ продаж магазина](#) | Аналитика продаж: топ товаров, динамика по месяцам | PostgreSQL |
| [🗄️ База сотрудников](#) | Проектирование БД, ER-диаграмма, сложные запросы | MySQL |
| [📈 Дашборд метрик](#) | Расчёт retention и когортный анализ | SQL + Excel |



\---



\## 🎓 Обучение


\- 🏫 Студент 4 курса колледжа по специальности **«Информационные системы и программирование»**

\- 📘 \[shipthatcode] — SQL Fundamentals; SQL Intermediate; SQL Advanced(4 of 7 lessons)

\- 💻 \[SQL-EX / LeetCode / Stepik / SQL Practice: Learn Database] — практика задач

\- 🎯 Сейчас изучаю: \*\*оконные функции и оптимизация запросов\*\*



\---



\## 📫 Связаться со мной


[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/MonamU3)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:gleb.yulegin@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/тлеб-юлегин-6848b6436)



\---



⭐ \*Открыт к стажировкам и junior-позициям в аналитике данных!\*

